...
export const deleteMessage = async (messageId) => {
  try {
    ...
    // update the zustand chat store correctly
    const state = useChatStore.getState();
    const prev = state.messages || [];
    state.setMessages(Array.isArray(prev) ? prev.filter((m) => m.id !== messageId) : []);
    return res.data;
  } catch (err) {
...
export const clearAllMessages = async (email) => {
  try {
    ...
    handleApiError(err, "Failed to clear messages.");
    throw err;
  }
};
